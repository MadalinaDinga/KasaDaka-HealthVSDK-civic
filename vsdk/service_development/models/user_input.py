...
class SpokenUserInput(models.Model):
    ...
    def __str__(self):
        ...
        return _('Spoken User Input: %(category_name)s @ %(date)s %(time)s by %(caller_id)s (%(service_name)s)') % {
            'category_name': self.category.name, 'date': str(date), 'time': str(time),
            'caller_id': str(self.session.caller_id), 'service_name': self.session.service.name}
